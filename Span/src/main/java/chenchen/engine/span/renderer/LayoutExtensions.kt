 ...
 * 获取当前行的行首索引
 */
fun Layout?.getLineStartOrUnknown(position: Int): Int {
    if (position <= 0) return position
    return this?.getLineStart(position) ?: -1
}

/**
 * 获取当前行的行尾索引
 */
fun Layout?.getLineEndOrUnknown(position: Int): Int {
    if (position <= 0) return position
    return this?.getLineEnd(position) ?: -1
}

/**
 * 获取索引所在的行
 */
fun Layout?.getLineForOffsetOrUnknown(position: Int): Int {
    if (position <= 0) return position
    return this?.getLineForOffset(position) ?: -1
}